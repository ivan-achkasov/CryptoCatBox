...
class BinancePerpetualCandlesAutoUpdateService(
    ...
    private val webSocketClient: cryptocatbox.binance.infrastructure.client.BinanceFuturesWebSocketClient,
    ...
    private suspend fun init() {
        val pairs = futurePairsProvider.getAllowedPairs()
            ...
            .toSet()
        connection.sendMessage(cryptocatbox.binance.infrastructure.client.FuturesWebSocketMessages.subscribePerpetual1MinCandles(pairs))
        pairs.forEach { pair ->
        ...
        updateDataThreadPool.execute {
            rawData.readCandleValue()?.let {
                try {
                    ...
                    connection.sendMessage(cryptocatbox.binance.infrastructure.client.FuturesWebSocketMessages.unsubscribePerpetual1MinCandles(it.pair))
                }
            }
        }
    }
    ...
    private fun String.readCandleValue(): Candle? {
        ...
        val dto = objectMapper.readValue(this, object : TypeReference<cryptocatbox.binance.infrastructure.client.KLineDto>() {})
        if (dto == null) {
            return null
        }
